# Rarity scoring and weekly Census Notice — REJECTED

**Status:** Rejected (2026-05-10) — superseded by [`../rarity-and-census.md`](../rarity-and-census.md)
...
This spec defined rarity as **trait-distribution surprisal** — a per-creature score derived from how unusual its glyph/anatomy/colour features are relative to the catalogue. That framing answers "is this creature visually unusual?" but does not answer the question we actually wanted: **"have many people found this species, or only a few?"**.

The replacement spec ([`../rarity-and-census.md`](../rarity-and-census.md)) reframes rarity around `species_discoveries.discovery_count`: a specimen is rare when few explorers have found it, and stops being rare as the catalogue fills. Tier is derived live at every read — no `rarity_at_discovery` snapshot.

**What was kept:**
- Three-tier vocabulary (rebadged: Extraordinary / Notable / Common)
- The Society's tone for tier-related Gazette copy
- The principle that field-note prose is timeless (never references current tier)

**What was rejected:**
- Trait-distribution surprisal scoring and the `species.rarity_score` column
- The weekly **Census Notice** cron and batch recalculation — replaced by per-event `tier_change` posts that fire organically from `register_discovery` only when a threshold is crossed
- `rarity_at_discovery` snapshot semantics — explicitly rejected; tier reflects the world *now*
- Anti-gameability defences — out of scope under the project's single-trusted-team threat model

**Schema mismatch (separate concern):** This spec also referenced fictional `species` and `discoveries` tables. Production uses `creatures`, `species_images`, and `species_discoveries`. The replacement spec was written against the actual schema.

The original spec is preserved below as historical record.
