...
| `FeaturedDispatch` | Per-day "Dispatch of the Day" — eyebrow, species `<h2>`, italic pull-quote, signature with time-of-day phrase. Mirrored layout via `sm:flex-row-reverse` when `mirrored=true`. First-discovery treatment: amber-tinted frame (`border-amber-700/40` + `ring-amber-500/20`), "⭐ New to science" eyebrow (replaces "Dispatch of the day" — the amber frame already signals the featured slot), "First sighting in the world." caption beneath the species name, signature reads "— first documented by **Name**" with bolder weight. |
| `CompactDispatch` | Thumb + italic species name + quote excerpt + signature. Full card is a button when `qr_hash` and `onViewSpecies` are provided. First-discovery treatment: left edge marker (`border-l-2 border-amber-700/40 pl-2`), bolder amber "⭐ First sighting" eyebrow (text-[11px], `font-semibold`), signature reads "— first documented by **Name** · time" with bolder discoverer name. |
