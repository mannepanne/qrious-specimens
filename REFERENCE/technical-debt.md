...
- **Risk:** Medium — increases as the RPC surface grows. A silent regression in `admin_delete_user_data` or any `get_*` RPC would ship to production until manual smoke testing catches it. Concrete precedent: issue #111 — PR #106 renamed `register_discovery`'s OUT parameter to `is_first_discoverer`, colliding with the table column in the embedded UPDATE. The function raised `column reference "is_first_discoverer" is ambiguous` at runtime on every call, rolling back species_discoveries inserts and tier_change events. The worker's soft-fail fallback swallowed the error, so no smoke test caught it until first-discoverer badges silently stopped appearing. A pgTAP assertion that calls `register_discovery` and reads back `creatures.is_first_discoverer` would have failed in CI.
