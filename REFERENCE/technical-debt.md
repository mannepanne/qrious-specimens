...
- **Status:** Resolved alongside TD-026 by extracting the allowlist into the `ALLOWED_ORIGINS` env var (set in `wrangler.toml` `[vars]` to `"https://qrious.hultberg.org"`). All three Workers read it via the `workers/shared/cors.ts` helper, so the production deployment automatically excludes localhost. The fallback path inside `parseAllowedOrigins()` returns the canonical production origin when the var is missing or empty, keeping existing local `vitest` runs (which don't set the var) on the prod-default behaviour. The fallback is fail-safe rather than fail-closed by design: a misconfigured deploy keeps serving prod traffic against the canonical origin instead of bricking every CORS request, and the worst-case outcome of a typo is "tighter than intended" rather than "more permissive".
